...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState({ regions: 0, drivers: 0, vehicles: 0 });
  ...
  const fetchHierarchy = async (token) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        // Count vehicles by traversing the tree
        let vehicleCount = 0;
        const countVehicles = (nodes) => {
          for (const node of nodes) {
            if (
              node.role === "vehicle" ||
              node.attributes?.role === "vehicle"
            ) {
              vehicleCount++;
            }
            if (node.children && node.children.length > 0) {
              countVehicles(node.children);
            }
          }
        };
        countVehicles(data);
  ...
  const renderCustomNode = ({ nodeDatum, toggleNode }) => {
    ...
    const displayEmail =
      nodeDatum.email || nodeDatum.attributes?.email || "no-email@system.com";
    ...
    } else if (rawRole.includes("vehicle")) {
    ...
    // For vehicles, show model instead of email
    const displayMetadata = rawRole.includes("vehicle")
      ? nodeDatum.attributes?.model || "Unknown Model"
      : displayEmail;
      ...
      <foreignObject width="240" height="120" x="-120" y="-60">
        ...
        >
          ...
          <div className="flex justify-between items-start">
            <div
              className={`flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${bgColor} ${textColor}`}
            >
              {icon}
              <span>{label}</span>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden max-w-4xl w-full flex flex-col md:flex-row border border-slate-100">
          <div className="bg-slate-900 p-12 md:w-2/5 text-white flex flex-col justify-center relative overflow-hidden">
            ...
            <h1 className="text-3xl font-bold mb-2 tracking-tight">
              Vendor<span className="text-blue-400">OS</span>
            </h1>
            <p className="text-slate-400 text-sm">
              Next-Gen Fleet Hierarchy System
            </p>
          </div>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-slate-800 text-sm"
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-lg transition shadow-lg shadow-blue-600/20 text-sm"
              >
      ...
      <nav className="bg-white border-b border-slate-200 px-6 h-16 flex justify-between items-center shrink-0 z-20">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-slate-800 tracking-tight">
            Vendor<span className="text-blue-600">OS</span>
          </h1>
        </div>
        <button
          onClick={handleLogout}
          className="text-slate-500 hover:text-red-600 flex items-center gap-2 text-sm font-medium transition bg-slate-50 px-3 py-1.5 rounded-md border border-slate-200 hover:border-red-200 hover:bg-red-50"
        >
